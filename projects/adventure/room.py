...
#                   q = Queue()    visited = set()    q.enqueue([player.currentRoom.id])     while q.size() > 0: path = q.dequeue()        last_room = path[-1]         if last_room not in visited: visited.add(last_room)             for exit in graph[last_room]: if graph[last_room][exit] == "?": return path                 else: dup_path = list(path)                    dup_path.append(graph[last_room][exit])                    q.enqueue(dup_path)     return [] 
